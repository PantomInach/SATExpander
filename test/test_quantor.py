...
class TestOperator(unittest.TestCase):
    def test_operator_add_suboperator(self):
        operator1 = LogicalOperator(LogicalOperatorType.ALL, None, ())
        operator2 = LogicalOperator(LogicalOperatorType.ALL, None, ())
        operator1.add_suboperator(operator2)
        self.assertEqual(operator1.suboperator, operator2)
        operator3 = LogicalOperator(LogicalOperatorType.ALL, None, ())
        operator4 = LogicalOperator(LogicalOperatorType.EXISTS, None, ())
        operator3.add_suboperator(operator4)
        self.assertEqual(operator3.suboperator, operator4)
        operator5 = LogicalOperator(LogicalOperatorType.ALL, None, ())
        operator6 = DummyExpressionOperator(0, 0)
        operator5.add_suboperator(operator6)
        self.assertEqual(operator5.suboperator, operator6)
        operator7 = LogicalOperator(LogicalOperatorType.EXISTS, None, ())
        operator8 = LogicalOperator(LogicalOperatorType.ALL, None, ())
        with self.assertRaises(RuntimeError) as _:
            operator7.add_suboperator(operator8)
        operator9 = LogicalOperator(LogicalOperatorType.EXISTS, None, ())
        operator10 = LogicalOperator(LogicalOperatorType.EXISTS, None, ())
        operator9.add_suboperator(operator10)
        self.assertEqual(operator9.suboperator, operator10)
        operator11 = LogicalOperator(LogicalOperatorType.EXISTS, None, ())
    ...
    def test_operator_chain(self):
        operator1 = LogicalOperator(LogicalOperatorType.ALL, None, ())
        operator2 = LogicalOperator(LogicalOperatorType.ALL, None, ())
        operator3 = LogicalOperator(LogicalOperatorType.EXISTS, None, ())
        operator4 = LogicalOperator(LogicalOperatorType.EXISTS, None, ())
        ...
        with self.assertRaises(RuntimeError) as _:
            operator1.chain(LogicalOperator(LogicalOperatorType.EXPRESSION, None, ()))
        with self.assertRaises(RuntimeError) as _:
            operator2.chain(LogicalOperator(LogicalOperatorType.ALL, None, ()))
        with self.assertRaises(RuntimeError) as _:
            operator3.chain(LogicalOperator(LogicalOperatorType.EXISTS, None, ()))
        operator6 = LogicalOperator(LogicalOperatorType.ALL, None, ())
        operator7 = LogicalOperator(LogicalOperatorType.ALL, None, ())
        operator8 = LogicalOperator(LogicalOperatorType.EXISTS, None, ())
        operator9 = LogicalOperator(LogicalOperatorType.EXISTS, None, ())
