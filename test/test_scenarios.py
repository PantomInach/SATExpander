...
from sat_expander.ExclusionPredicates import (
    check_variables_in_context,
    VarNotFoundResponse,
)
...
class TestScenarios(unittest.TestCase):
    def test_scenario1(self):
        ...
        quant = (
            AndOperator(("x",), to_tuple_iter(base_set))
            .chain(OrOperator(("y",), to_tuple_iter(base_set)))
            .chain(ExpressionOperator(factory, ("f(x, y)",)))
        )
        f = func.relation
        expected_res = (
            (f[(1, 1)], f[(1, 2)], f[(1, 3)]),
    ...
    def test_scenario2(self):
        ...
        quant = (
            AndOperator(("x", "y"), zip(base_set1, base_set2))
            .chain(AndOperator(("z"), to_tuple_iter(base_set1)))
            .chain(ExpressionOperator(factory, ("-ff(z,y)", "gg(x)")))
        )
        f = func1.relation
        g = func2.relation
        expected_result = (
    ...
    def test_scenario3(self):
        ...
        quant = (
            AndOperator(("x", "y"), zip(base_set1, base_set2))
            .chain(AndOperator(("z",), to_tuple_iter(base_set1)))
            .chain(ExpressionOperator(factory, ("-ff(z,y)", "ff(x,y)")))
        )
        f = func1.relation
        expected_result = (
            (-f[(1, "aa")], f[(1, "aa")]),
    ...
    def test_scenario4(self):
        ...
        expected_result = ((1, -f[("hi",)], 1, -f[("bye",)]),)
    ...
    def test_scenario5(self):
        ...
        @check_variables_in_context(
            "x", "y", var_not_found_response=VarNotFoundResponse.ERROR
        )
    ...
    def test_scenario6(self):
        ...
        quant = (
            ...
            .chain(
                OrOperator(
                    ("y",), to_tuple_iter(base_set), exclusion_predicate=predicate
                )
            )
            .chain(ExpressionOperator(factory, ("f(x,y)",)))
        )
